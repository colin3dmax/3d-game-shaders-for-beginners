...
for f in $SCRIPT_PATH/../sections-zh_cn/*
do
  echo "$f"
  file=$(basename -- "$f")
  file_name="${file%.*}"
...
do
  ...
  do
    ...
    then
      echo $f
      sed -i '.bak' -E 's|href="(sections/)?([a-z-]+)\.md([^"]*)"|href="\2.html\3"|g' $f
      sed -i -E 's/href="\.\.\/README-zh_cn.md"/href="index.html"/g' $f
      sed -i '.bak' -E 's|<a href="\.\./demonstration/([^"]*)">|<a href="'"$REPO_URL"'/blob/master/demonstration/\1" target="_blank" rel="noopener noreferrer">|g' "$f"

      rm -f "$f.bak"  # 可选：删除备份文件
       rm -f "$f-E"  # 可选：删除备份文件
    fi
  done
done
